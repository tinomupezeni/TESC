...
import {
  Search,
  BookOpen, // Example icon, maybe change to something more related to inclusivity
  Users,
  Loader2,
  Upload,
} from "lucide-react";
...
const Inclusivity = () => {
  ...
  const [inclusivityStudents, setInclusivityStudents] = useState<InclusivityStudent[]>([]);
  ...
  const fetchInclusivityStudents = useCallback(async () => {
    if (!user?.institution?.id) return;
    try {
      setLoading(true);
      const apiResponse: InclusivityStudentsAPIResponse = await getInclusivityStudents(
        user.institution.id,
        searchQuery
      );
      setInclusivityStudents(apiResponse.results);
      ...
      toast.error("Failed to load Inclusivity students");
      setInclusivityStudents([]);
      setTotalStudents(0);
      setMaleStudents(0);
      setFemaleStudents(0);
    } finally {
  ...
  useEffect(() => {
    fetchInclusivityStudents();
  }, [fetchInclusivityStudents]);

  const filteredInclusivityStudents = inclusivityStudents; // Filtering already applied by fetchInclusivityStudents
    ...
    <div className="space-y-6">
      ...
      <div className="px-1">
        ...
        <p className="text-sm sm:text-base text-muted-foreground mt-1">
          View and manage students by inclusivity category for{" "}
              ...
              <div className="flex-1 sm:flex-none">
                <Dialog>
                  ...
                  <DialogContent className="max-w-4xl">
                    <DialogHeader><DialogTitle>Bulk Upload Inclusivity Data</DialogTitle></DialogHeader>
                    <BulkUploadResolver moduleType="inclusivity" onSuccess={fetchInclusivityStudents} />
                  </DialogContent>
                </Dialog>
              </div>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 mb-6">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Total Inclusivity</CardTitle>
                ...
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={6} className="h-24 text-center">
                      <Loader2 className="mr-2 h-4 w-4 animate-spin inline" /> Loading inclusivity data...
                    </TableCell>
                  </TableRow>
                ) : filteredInclusivityStudents.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8 text-muted-foreground text-xs">
                      No inclusivity students found.
                    </TableCell>
                  </TableRow>
                ) : (
                  filteredInclusivityStudents.map((student) => (
                    <TableRow key={student.id}>
                      ...
                      <TableCell className="text-xs">{student.inclusivity_category}</TableCell>
