...
import { SpecializedStudent, SpecializedStudentsAPIResponse, getSpecializedStudents } from "../services/reports.services";

const SpecializedStudents = () => {
  const { user } = useAuth();
  
  const [students, setStudents] = useState<SpecializedStudent[]>([]);
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const apiResponse: SpecializedStudentsAPIResponse = await getSpecializedStudents(
        user.institution.id,
        searchQuery
      );
      ...
      console.error("Failed to fetch specialized students", error);
      toast.error("Failed to load specialized students");
      setStudents([]);
      setTotalStudents(0);
      setMaleStudents(0);
      setFemaleStudents(0);
    ...
    <div className="space-y-6">
      ...
      <div className="px-1">
        <h1 className="text-2xl sm:text-3xl font-bold text-foreground tracking-tight">
          Specialized Skills Students
        </h1>
        <p className="text-sm sm:text-base text-muted-foreground mt-1">
          View and manage students enrolled in specialized skill programs for{" "}
      ...
      <Card className="overflow-hidden border-none sm:border">
        <CardHeader className="p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <CardTitle className="text-lg sm:text-xl">Specialized Skills Directory</CardTitle>
              <CardDescription className="text-xs sm:text-sm">Comprehensive list of specialized program students</CardDescription>
            </div>
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
              <AddStudentDialog onStudentAdded={fetchStudents} defaultCategory="SPECIALIZED" />
              <div className="flex-1 sm:flex-none">
                <Dialog>
                  ...
                  <DialogContent className="max-w-4xl">
                    <DialogHeader><DialogTitle>Bulk Upload Specialized Skills Students</DialogTitle></DialogHeader>
                    <BulkUploadResolver moduleType="specialized_students" onSuccess={fetchStudents} />
                  </DialogContent>
                </Dialog>
              </div>
            </div>
                ...
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={5} className="h-24 text-center">
                      <Loader2 className="mr-2 h-4 w-4 animate-spin inline" /> Loading specialized students...
                    </TableCell>
                  </TableRow>
                ) : students.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-8 text-muted-foreground text-xs">
                      No specialized students found.
                    </TableCell>
                  </TableRow>
                ) : (
...
export default SpecializedStudents;
