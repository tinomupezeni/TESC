...
import { Student, updateStudent, UpdateStudentData } from "@/services/students.services";
...
export const EditStudentDialog: React.FC<EditStudentDialogProps> = ({
    ...
    if (student) {
      ...
      const loadAcademicData = async (instId: number) => {
        ...
        try {
          ...
          const currentProgram = progData?.find(p => p.id === student.program);
          if (currentProgram) {
            ...
            const currentDept = deptData?.find(d => d.id.toString() === currentProgram.department.toString());
            if (currentDept) {
  ...
  const handleSelectChange = (field: keyof UpdateStudentData, value: string) => {
    ...
    if (field === 'status') {
        if (value !== 'Graduated') {
            setFormData(prev => ({ ...prev, graduation_year: null, final_grade: null }));
        }
        if (value !== 'Dropout') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        // Prepare data for API call (ensure only changed data is sent if using PATCH fully)
        const dataToSend: UpdateStudentData = {
            ...formData,
            // Ensure program is sent as number ID
    ...
    <Dialog open={!!student} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the profile for **{student?.full_name}** ({student?.student_id})
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4 p-4 border rounded-lg">
            <h3 className="text-lg font-semibold border-b pb-2">Personal & ID Information</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              
              <div className="space-y-2">
                <Label htmlFor="first_name">First Name *</Label>
                <Input id="first_name" value={formData.first_name} required onChange={handleInputChange} disabled={isLoading} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="last_name">Last Name *</Label>
                <Input id="last_name" value={formData.last_name} required onChange={handleInputChange} disabled={isLoading} />
              </div>
              
              <div className="space-y-2">
                <Label htmlFor="student_id">Student ID *</Label>
                <Input id="student_id" value={formData.student_id} required onChange={handleInputChange} disabled={isLoading} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="national_id">National ID</Label>
                <Input id="national_id" value={formData.national_id} onChange={handleInputChange} disabled={isLoading} />
              </div>
              
              <div className="space-y-2">
                <Label htmlFor="gender">Gender *</Label>
                <Select onValueChange={(val) => handleSelectChange('gender', val)} value={formData.gender} required disabled={isLoading}>
              ...
              <div className="space-y-2">
                ...
                <Input id="date_of_birth" type="date" value={formData.date_of_birth} onChange={handleInputChange} disabled={isLoading} />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 p-4 border rounded-lg bg-muted/20">
            <h3 className="text-lg font-semibold border-b pb-2">Academic Placement</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                    ...
                    <Select 
                        ...
                        disabled={isLoading || !selectedFacultyId || filteredDepartments.length === 0}
                    >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                    ...
                    <Select 
                        ...
                        disabled={isLoading || !selectedDeptId || filteredPrograms.length === 0}
                        required
                    >
                    <SelectTrigger>
          ...
          <div className="space-y-4 p-4 border rounded-lg">
            <h3 className="text-lg font-semibold border-b pb-2">Enrollment & Financial Status</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {formData.status === 'Graduated' && (
                <div className="grid grid-cols-2 gap-4 p-3 border border-green-500/50 rounded-md bg-green-500/10">
                    ...
                    <div className="space-y-2">
                        ...
                        <Select 
                            onValueChange={(val) => handleSelectChange('final_grade', val)} 
                            value={formData.final_grade || ''} 
                            disabled={isLoading}
                        >
                            <SelectTrigger><SelectValue placeholder="Select final grade" /></SelectTrigger>
                            <SelectContent>
            ...
            {formData.status === 'Dropout' && (
                <div className="space-y-2 p-3 border border-red-500/50 rounded-md bg-red-500/10">
                    ...
                    <Select 
                        onValueChange={(val) => handleSelectChange('dropout_reason', val)} 
