...
export default function RegisterInst({
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // --- Invalidate queries so statistics page updates automatically ---
      queryClient.invalidateQueries({ queryKey: ["institutions"] });

      if (isEditMode) {
        queryClient.invalidateQueries({ queryKey: ["institution", institutionToEdit.id] });
      }
  ...
  const handleFacilityChange = (facilityId: number, checked: boolean) => {
    ...
    if (checked) newSet.add(facilityId);
    else newSet.delete(facilityId);
